/**
 * Smoke test: Others List (GetTransaction type O, edit).
 * Usage: TEST_USER=... TEST_PASS=... node scripts/test-others-list.mjs
 */
...
async function main() {
  ...
  console.log("LIST O:", list.data?.result, "rows:", list.data?.data?.transactions?.length ?? 0);
  if (!list.data?.result) {
  ...
  if (rows.length === 0) {
    console.log("No others rows — list API OK");
    return;
  }
