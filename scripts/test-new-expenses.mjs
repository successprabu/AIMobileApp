/**
 * Smoke test: New Expenses (Transaction/UpdateTransaction type E).
 * Usage: TEST_USER=... TEST_PASS=... node scripts/test-new-expenses.mjs
 */
...
async function main() {
  ...
  const payload = {
    ...
    villageName: "catering",
    initial: "",
    name: "API Expenses Test Person",
    oldAmount: 0,
    newAmount: 0,
    amount: 2500,
  ...
  const list = await axios.get(`${BASE}Transaction/GetTransaction`, {
    params: {
      ...
      customer_name: "API Expenses Test",
      trans_type: "E",
      village_name: "",
      mobile: "",
  ...
  const found = (list.data?.data?.transactions ?? []).some(
    (t) => t.name === "API Expenses Test Person" && t.villageName === "catering"
  );
