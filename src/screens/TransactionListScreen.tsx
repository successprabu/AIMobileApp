...
export default function TransactionListScreen() {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({ title: t("transactionList") });
  }, [navigation, t]);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const deletedBy = String(u.userName ?? u.name ?? u.id ?? "system");
  ...
  const handleEditSave = async (record: TransactionRecord) => {
    ...
    if (res.result) {
      setRows((prev) => prev.map((r) => (r.id === record.id ? record : r)));
    ...
    <Card style={styles.rowCard} mode="outlined">
      <Card.Content>
        ...
        <Text variant="bodySmall">
          {t("phoneNo")}: {item.phoneNo || "—"} · {t("active")}: {item.isActive ? t("yes") : t("no")}
        </Text>
        <View style={styles.rowActions}>
          <IconButton icon="pencil" onPress={() => setEditing(item)} />
          <IconButton icon="delete" iconColor="#c62828" onPress={() => setDeleteId(item.id)} />
        </View>
      </Card.Content>
    </Card>
  );
